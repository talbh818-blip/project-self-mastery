...
export function HabitDescription({
  ...
  if (format === 'text') {
    return (
      <p
        className={`text-[13px] text-ink-300 leading-relaxed whitespace-pre-wrap ${className}`}
      >
        {description}
      </p>
    );
  ...
  return (
    <ul className={`space-y-1 ${className}`}>
      {items.map((item, i) => (
        <li
          key={i}
          className="flex items-start gap-2 text-[13px] text-ink-300 leading-snug"
        >
          <span className="shrink-0 text-ink-500 tabular-nums mt-px select-none">
